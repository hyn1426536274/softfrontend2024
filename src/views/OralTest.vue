  ...
  <div class="chat-container">
    <div class="chat-messages" ref="messagesContainer">
      <div v-for="message in messages" :key="message" :class="message.type === 'user' ? 'user-message' : 'bot-message'">
        <div :class="message.score > 60 ? 'score_high' : 'score_low'">
          {{ message.score }}
        </div>
    ...
    <!-- <button class="chatbtn" @click="chat">
      click me
    </button> -->
...
export default {
  ...
  data() {
    return {
      messages: [
        { type: 'bot', text: '### You can have an English conversation with me, and I will grade your pronunciation  (你可以和我进行英文对话，我会为你的发音评分)', score: 0 },
        // { type: 'user', text: 'Hello', score: 66 },
        // { type: 'bot', text: '## Hi, how can I help you?', score: 0 },
        // { type: 'user', text: 'I want to know about your products', score: 50 },
        // { type: 'bot', text: '**Sure**, ---  what do you want to know? --- ', score: 0 }
      ],
      siriwave: null,
      isRecording: true,
      maxchat: 5,
  ...
  methods: {
    ...
    async audioToText() {
      try {
        ...
        await this.voiceScore(); // 语音评分

        this.messages.push({ type: 'user', text: this.current_question, score: this.calculatePercentage(this.current_score, 5) });
    ...
    async sparkChat() {
      try {
        ...
        this.messages.push({ type: 'bot', text: this.current_answer, score: 0 });
    ...
    async voiceScore() {
      try {
        const formData = new FormData();
        formData.append('ENT', 'en_vip');
        formData.append('CATEGORY', 'read_sentence');
        formData.append('TEXT', this.current_question);
        formData.append('AudioFile', this.audiopath);
        const response = await axios.post('http://127.0.0.1:8000/语音评测', formData,
          {
            headers: {
              // 'Content-Type': 'multipart/form-data'
              'Content-Type': 'application/json'
        ...
        console.log('score', response.data.result);
        this.current_score = response.data.result['总分'];
      }
      catch (error) {
        console.error(error);
      }
...
.chat-container {
  ...
  height: 100vh;
  width: 100%;
  background-image: url('../../public/images/waves/wave08.svg');
...
.status_note{
  ...
  font-size: 18px;
  font-weight: bold;
  background-color: rgba(197, 255, 255, 0.532);
}
